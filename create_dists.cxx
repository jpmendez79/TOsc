...
TFile outfile(tmp_name, "RECREATE");
  ...
  vector<double> vec_chi2_3v_with_3vToy(num_toys);
  vector<double> vec_chi2_4v_with_3vToy(num_toys);
  vector<double> vec_dchi2_3v(num_toys);
  vector<double> vec_chi2_3v_with_4vToy(num_toys);
  vector<double> vec_chi2_4v_with_4vToy(num_toys);
  vector<double> vec_dchi2_4v(num_toys);
  cout << "Generate 3v\n";
  // Generate 3v Toys
  ...
  for (int i = 0; i < num_toys; i++) {
    ...
    vec_chi2_3v_with_3vToy[i] = chi2_3v;
    vec_chi2_4v_with_3vToy[i] = chi2_4v;
    vec_dchi2_3v[i] = dchi2;
  }
  cout << "Generate 4v\n";
  // Generate 4v Toys
  ...
  for (int i = 0; i < num_toys; i++) {
    ...
    double chi2_4v = osc_test->FCN(pars_4v_grid);
    double dchi2 = chi2_4v - chi2_3v;
    vec_chi2_3v_with_4vToy[i] = chi2_3v;
    vec_chi2_4v_with_4vToy[i] = chi2_4v;
    vec_dchi2_4v[i] = dchi2;
  }
